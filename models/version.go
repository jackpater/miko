...
func Update(sender *Sender) error {
	sender.Reply("Miko开始拉取代码")
	rtn, err := exec.Command("sh", "-c", "cd "+ExecPath+" && git stash && git pull").Output()
	if err != nil {
		return errors.New("Miko拉取代失败：" + err.Error())
	}
	t := string(rtn)
	if !strings.Contains(t, "changed") {
		if strings.Contains(t, "Already") || strings.Contains(t, "已经是最新") {
			return errors.New("Miko已是最新版啦")
		} else {
			return errors.New("Miko拉取代失败：" + t)
		}
	} else {
		sender.Reply("Miko拉取代码成功")
	}
	sender.Reply("Miko正在编译程序")
	rtn, err = exec.Command("sh", "-c", "cd "+ExecPath+" && go build -o "+pname).Output()
	if err != nil {
		return errors.New("Miko编译失败：" + err.Error())
	} else {
		sender.Reply("Miko编译成功")
	}
	return nil
}
